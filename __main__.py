...
async def bot_runner(bot: BaseBot, room_id: str, api_key: str) -> None:
    async with TaskGroup() as tg:
        # PATCH: Throttler auf 1 Versuch / 60 Sekunden geändert
        # Original: throttler(5, 5) → max 5 Reconnects in 5s = viel zu aggressiv
        # Neu: throttler(1, 60) → max 1 Versuch, dann 60s warten
        t = throttler(1, 60)
        consecutive_fails = 0
        while True:
            await anext(t)
            ...
            try:
                async with ClientSession() as session:
                    ...
                    async with session.ws_connect(
                        ...
                        if isinstance(session_metadata, Error):
                            ...
                            ka_task.cancel()
                            # PATCH: Bei Fehler (z.B. "already added") länger warten
                            consecutive_fails += 1
                            wait = min(60 * consecutive_fails, 300)
                            print(f"ERROR: Warte {wait}s vor nächstem Versuch (Fail #{consecutive_fails})...")
                            await sleep(wait)
                            continue
                        consecutive_fails = 0  # Reset bei erfolgreicher Verbindung
                ...
                consecutive_fails += 1
                # PATCH: Exponentieller Backoff statt sofortigem Reconnect
                wait = min(30 * consecutive_fails, 300)
                print(f"ERROR: reconnecting... Warte {wait}s (Fail #{consecutive_fails})")
                await sleep(wait)
